...
class Configuration(commands.Cog, name = "Bot Configuration Command"):
	...
	async def send_modal(self, ctx : commands.Context, channel : nextcord.TextChannel = None):
		...
		embed = nextcord.Embed(
			...
			color = nextcord.Color.from_rgb(163, 128, 255)
		).set_image(url="https://media.discordapp.net/attachments/919251928754585651/1065231467099213854/232.png?width=1440&height=1")

		await ctx.reply(sent_button_to_channel_message)
		await channel.send(embed = embed, view = FormView())

	#hui
	@commands.command(name = "sendimage", description = "for apps")
	@commands.has_permissions(administrator = True)
	async def config_bot(self, ctx : commands.Context, channel : nextcord.TextChannel = None):
		embedimg = nextcord.Embed(
			color = nextcord.Color.from_rgb(163, 128, 255)
		).set_image(url="https://media.discordapp.net/attachments/919251928754585651/1065231467099213854/232.png?width=1440&height=420")

		await channel.send(embed = embedimg)
